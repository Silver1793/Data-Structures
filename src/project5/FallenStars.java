...
/**
 * The main method for project5. Reads an inputed file and gives errors if the file is unreadable or missing. 
 * Adds all the elements from the given file into an MeteoriteData of type meteorite. Then asks the user to 
 * search through the MeteoriteData by entering a mass, year or location. Returns a MeteoriteData for elements within the same mass range +- 10,
 * a MeteoriteData for elements with the same inputed year or an element that is closest to the inputed location.
...
public class FallenStars { 
	...
	public static void main(String[] args) {
		...
		MeteoriteData metData = new MeteoriteData();
		Location loc = null;
		...
		while(inMet.hasNextLine()) 
		{
			...
			try {
				m = new Meteorite(splitCSVLine(line).get(0), Integer.parseInt(splitCSVLine(line).get(1)));
	  ...
	  To finish the program, enter
		...
		do {
			...
			if(arr[0].equalsIgnoreCase("mass"))
			{
				...
				if(metData.getByMass(mass,10) == null)
				{
					System.out.println("No elements were found for the inputted mass.");
					continue;
				}
				System.out.println(metData.getByMass(mass, 10));
				continue;
				}
				catch(NumberFormatException e)
				{
			...
			if(arr[0].equalsIgnoreCase("year"))
			{
				try {				
					...
					if(metData.getByYear(year) == null)
					{
						System.out.println("No elements were found for the inputted year.");
						continue;
					}
					System.out.println(metData.getByYear(year));
					continue;
				}
				catch(NumberFormatException e)
				{
			...
			if(arr[0].equalsIgnoreCase("location"))
			{
				try
				{
				double value3 = Double.parseDouble(arr[1]);
				double value4 = Double.parseDouble(arr[2]);
				loc = new Location(value3, value4);
				//System.out.println(metData);
				System.out.println(metData.getByLocation(loc));
				}
				catch(IllegalArgumentException e) {
					System.err.println("Illegal parameter, Latitude must be between -90 and 90 and longitude but be between -180 and 180");
					continue;
				}
				/*catch(NumberFormatException e)
				{
